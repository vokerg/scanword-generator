...
function fixture() {
  return {
    ...
    attempt: 0,
    coverageCheckpoint: { minimumAnswers: 1, maximumPanels: 10, requiredComponents: 1 },
  };
}
...
global.ScanwordClosedFill = {
  measureCoverage(grid) {
    ...
    return { panelCells, letterCells, totalCells: flat.length, activeCoverage: 0.4, answerSpaceCoverage: 0.4, rawLetterCoverage: 0.4 };
  },
};
global.ScanwordSolver = {
  ...
  resultMetrics(result) {
    return { validation: result.validation, components: result.components, score: 0, intersections: 0, doubles: 0, panelRegions: 1, isolatedPanels: 0, largestPanelRegion: 3 };
  },
};
...
assert.deepEqual(result.constructionV2.explicitStageRuntime.stages.map((stage) => stage.name), [
...
console.log(JSON.stringify({ passed: true, stages: result.constructionV2.explicitStageRuntime.stages.length }));
