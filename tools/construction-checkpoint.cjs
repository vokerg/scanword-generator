    ...
    child.on("close", (code) => {
      ...
      try {
        ...
        samples[index] = {
          ...
          targetedExactSelectedAtomicRelation: exact?.selected?.atomicPairRelation || null,
          ...
          targetedExactAtomicCrossingPairs: Number(atomic?.crossingSlotPairs || 0),
          targetedExactAtomicDisjointPairs: Number(atomic?.disjointSlotPairs || 0),
          targetedExactAtomicComponentPrunedPairs: Number(atomic?.componentPrunedPairs || 0),
          targetedExactAtomicMaximumRollbackComponents: Number(atomic?.maximumRollbackComponents || 0),
...
(async () => {
  try {
    ...
    const summary = {
      ...
      targetedExactAtomicCrossingSelectedSeeds: samples.filter((sample) => sample.targetedExactSelectedAtomicRelation === "crossing").length,
      targetedExactAtomicDisjointSelectedSeeds: samples.filter((sample) => sample.targetedExactSelectedAtomicRelation === "disjoint").length,
      ...
      totalTargetedExactAtomicCrossingPairs: samples.reduce((sum, sample) => sum + sample.targetedExactAtomicCrossingPairs, 0),
      totalTargetedExactAtomicDisjointPairs: samples.reduce((sum, sample) => sum + sample.targetedExactAtomicDisjointPairs, 0),
      totalTargetedExactAtomicComponentPrunedPairs: samples.reduce((sum, sample) => sum + sample.targetedExactAtomicComponentPrunedPairs, 0),
      maximumTargetedExactAtomicRollbackComponents: Math.max(...samples.map((sample) => sample.targetedExactAtomicMaximumRollbackComponents)),
