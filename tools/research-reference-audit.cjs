...
const manifestPath = path.join(root, "research", "closed-fill", "manifest.json");
const manifest = JSON.parse(fs.readFileSync(manifestPath, "utf8"));
const preservation = manifest.preservation || {};
const requiredRefs = Array.isArray(preservation.requiredRefs)
  ? preservation.requiredRefs
  : [];

const mainRef = preservation.mainRef || "refs/remotes/origin/main";
...
const branchPattern = /\b(?:r-and-d\/[A-Za-z0-9._/-]+|release\/[A-Za-z0-9._/-]+|research\/[A-Za-z0-9._-]+)\b/g;
...
const commits = uniqueSorted(commitFiles.keys()).map((sha) => {
  ...
  const inMain = available && runGit(["merge-base", "--is-ancestor", sha, mainRef], { cwd: root }).ok;
